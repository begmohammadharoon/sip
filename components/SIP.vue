<template>
    <div class="w-full h-full">
        ...
        <div class="flex flex-row px-4 py-4  justify-between">
            <div class="flex flex-col justify-center text-primary">Monthly Investment  &nbsp;</div>
            <span></span>
            <div>
        ...
        <div class="flex flex-col items-center">
            <input type="range" min="0" max="100000" step="500" v-model="p" class="range max-sm:w-72 hover:range-accent range-primary" />
        </div>
        <div class="flex flex-row px-4 py-4 justify-between">
...
const Total = computed(()=> {
    return (p.value*12*time.value).toLocaleString('en-IN');
});

const M = computed(()=> {
    let n = time.value*12;
    let i = (rate.value/100)/12;
    let power = Math.pow((1+i),n);

   return (Math.round(p.value * (((power)-1)/i) * (1+i))).toLocaleString('en-IN');
});
...
const Returns = computed(()=>{
    ...
    let i = (rate.value/100)/12;
    let power = Math.pow((1+i),n);
    let maturity = Math.round(p.value * (((power)-1)/i) * (1+i));
    
   return (maturity-p.value*time.value*12).toLocaleString('en-IN');
});
